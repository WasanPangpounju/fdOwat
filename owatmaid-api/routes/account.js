  ...
  try {
    ...
    for (const doc of records) {
      ...
      try {
        ...
        // ✅ คำนวณ dayType ใหม่ตามลำดับ priority (เหมือนกับ searchtimerecordemployee)
        if (doc.employee_record && doc.employee_record.length > 0) {
          for (let record of doc.employee_record) {
            try {
              ...
              console.log(`🏢 [accounting/searchtimerecordemployee] ข้อมูล workplace ${record.workplaceId} ทั้งหมด:`, JSON.stringify(workplace, null, 2));
              ...
              console.log(`🔍 [accounting/searchtimerecordemployee] ตรวจสอบ workplace ${record.workplaceId} วันที่ ${record.date} (${dayName}, dayOfWeek: ${dayOfWeek}) เดือน ${month} ปี ${year}`);
              ...
              if (record.workplaceId === '10493') {
                console.log(`🔧 [accounting/searchtimerecordemployee] workplace 10493: ตรวจสอบเฉพาะ dayOffOnly เท่านั้น`);
                ...
                try {
                  ...
                  console.log(`🔍 [accounting/searchtimerecordemployee] ตรวจสอบ dayOffOnly สำหรับ workplace ${record.workplaceId}:`, weekendData.dayOffOnly);
                  ...
                  if (weekendData && weekendData.dayOffOnly && weekendData.dayOffOnly.length > 0) {
                    ...
                    console.log(`📅 [accounting/searchtimerecordemployee] วันที่ต้องตรวจสอบ dayOffOnly: ${recordDateStr}`);
                    console.log(`📅 [accounting/searchtimerecordemployee] รายการ dayOffOnly:`, weekendData.dayOffOnly);
                    
                    const isDayOffOnly = weekendData.dayOffOnly.includes(recordDateStr);
                    console.log(`✅ [accounting/searchtimerecordemployee] ผลการตรวจสอบ dayOffOnly: ${isDayOffOnly}`);
                    
                    if (isDayOffOnly) {
                      console.log(`🚨 [accounting/searchtimerecordemployee] workplace 10493: วันที่ ${record.date} อยู่ใน dayOffOnly - เปลี่ยนเป็น stop`);
                      newDayType = 'stop';
                      newCashWorkMul = '2';
                    } else {
                      console.log(`✅ [accounting/searchtimerecordemployee] workplace 10493: วันที่ ${record.date} ไม่อยู่ใน dayOffOnly - เป็น work`);
                      newDayType = 'work';
                      newCashWorkMul = '1';
                    }
                  } else {
                    console.log(`⚠️ [accounting/searchtimerecordemployee] workplace 10493: ไม่มี dayOffOnly - เป็น work ทุกวัน`);
                    newDayType = 'work';
                    newCashWorkMul = '1';
                  }
                } catch (weekendError) {
                  console.error(`❌ [accounting/searchtimerecordemployee] ไม่สามารถดึงข้อมูล dayOffOnly สำหรับ workplace ${record.workplaceId}:`, weekendError.message);
                  // fallback สำหรับ 10493 ถ้าดึงข้อมูลไม่ได้
                  console.log(`� [accounting/searchtimerecordemployee] fallback สำหรับ workplace 10493: dayType = work`);
                  newDayType = 'work';
                  newCashWorkMul = '1';
                }
                ...
                if (record.dayType !== newDayType) {
                  console.log(`🔄 [accounting/searchtimerecordemployee] workplace 10493 อัปเดต dayType: ${record.dayType} -> ${newDayType}`);
                  record.dayType = newDayType;
                }
                
                if (record.cashWorkMul !== newCashWorkMul) {
                  console.log(`🔄 [accounting/searchtimerecordemployee] workplace 10493 อัปเดต cashWorkMul: ${record.cashWorkMul} -> ${newCashWorkMul}`);
              ...
              console.log(`🔍 [accounting/searchtimerecordemployee] ตรวจสอบ publicHoliday สำหรับ workplace ${record.workplaceId}:`, workplace.publicHoliday);
              
              if (workplace.publicHoliday && workplace.publicHoliday.length > 0) {
                const recordDateStr = `${year}-${month.padStart(2, '0')}-${record.date.padStart(2, '0')}`;
                console.log(`📅 [accounting/searchtimerecordemployee] วันที่ต้องตรวจสอบ: ${recordDateStr}`);
                ...
                const isPublicHoliday = workplace.publicHoliday.some(holiday => {
                  ...
                  console.log(`🗓️ [accounting/searchtimerecordemployee] เปรียบเทียบ holiday: ${holidayDateStr} กับ record: ${recordDateStr}`);
                  return holidayDateStr === recordDateStr;
                });
                
                console.log(`✅ [accounting/searchtimerecordemployee] ผลการตรวจสอบ publicHoliday: ${isPublicHoliday}`);
                
                if (isPublicHoliday) {
                  console.log(`🚨 [accounting/searchtimerecordemployee] PRIORITY 1: วันที่ ${record.date} เป็นวันหยุดนักขัตฤกษ์ (publicHoliday) - เปลี่ยนเป็น stop`);
                  newDayType = 'stop';
                  newCashWorkMul = '2';
                  
                  // อัปเดตค่าและข้ามการตรวจสอบอื่นๆ
                  if (record.dayType !== newDayType) {
                    console.log(`🔄 [accounting/searchtimerecordemployee] อัปเดต dayType: ${record.dayType} -> ${newDayType}`);
                    record.dayType = newDayType;
                  }
                  
                  if (record.cashWorkMul !== newCashWorkMul) {
                    console.log(`🔄 [accounting/searchtimerecordemployee] อัปเดต cashWorkMul: ${record.cashWorkMul} -> ${newCashWorkMul}`);
                    record.cashWorkMul = newCashWorkMul;
                  }
                ...
                console.log(`⚠️ [accounting/searchtimerecordemployee] workplace ${record.workplaceId} ไม่มี publicHoliday หรือเป็น array ว่าง`);
              }
              ...
              try {
                ...
                if (weekendData && weekendData.dayOffOnly && weekendData.dayOffOnly.length > 0) {
                  ...
                  if (isDayOffOnly) {
                    console.log(`🚨 [accounting/searchtimerecordemployee] PRIORITY 2: วันที่ ${record.date} อยู่ใน dayOffOnly - เปลี่ยนเป็น stop`);
                    newDayType = 'stop';
                    newCashWorkMul = '2';
                  ...
                  console.log(`⚠️ [accounting/searchtimerecordemployee] workplace ${record.workplaceId} ไม่มี dayOffOnly หรือเป็น array ว่าง`);
                }
              } catch (weekendError) {
                console.error(`❌ [accounting/searchtimerecordemployee] ไม่สามารถดึงข้อมูล dayOffOnly สำหรับ workplace ${record.workplaceId}:`, weekendError.message);
              }
              ...
              console.log(`🔍 [accounting/searchtimerecordemployee] ตรวจสอบ workTimeDay สำหรับ workplace ${record.workplaceId}:`, workplace.workTimeDay);
              ...
              if (workplace.workTimeDay && workplace.workTimeDay.length > 0) {
                ...
                workplace.workTimeDay.forEach(config => {
                  ...
                  console.log(`📝 [accounting/searchtimerecordemployee] Config: ${config.startDay}(${startDayIndex}) - ${config.endDay}(${endDayIndex}) = ${config.workOrStop}`);
                ...
                console.log(`📊 [accounting/searchtimerecordemployee] Work days: [${Array.from(workDays).sort()}], Stop days: [${Array.from(stopDays).sort()}]`);
                
                // ตรวจสอบว่าทุกวัน (0-6) อยู่ใน workDays หรือไม่
                const allDaysAreWork = [0, 1, 2, 3, 4, 5, 6].every(day => workDays.has(day));
                
                if (allDaysAreWork) {
                  console.log(`✅ [accounting/searchtimerecordemployee] พบการตั้งค่าทำงานทุกวัน (ครอบคลุม 0-6) - dayType = work`);
                  newDayType = 'work';
                  newCashWorkMul = '1';
                } else {
                  ...
                  if (workDays.has(dayOfWeek)) {
                    newDayType = 'work';
                    newCashWorkMul = '1';
                    console.log(`✅ [accounting/searchtimerecordemployee] วัน ${dayName} (${dayOfWeek}) ตั้งค่าเป็น work`);
                  } else if (stopDays.has(dayOfWeek)) {
                    newDayType = 'stop';
                    newCashWorkMul = '2';
                    console.log(`✅ [accounting/searchtimerecordemployee] วัน ${dayName} (${dayOfWeek}) ตั้งค่าเป็น stop`);
                  } else {
                    console.log(`⚠️ [accounting/searchtimerecordemployee] ไม่พบการตั้งค่าสำหรับวัน ${dayName} (${dayOfWeek}) - ใช้ค่าเริ่มต้น: work`);
                    newDayType = 'work';
                    newCashWorkMul = '1';
                  }
                }
              } else {
                console.log(`⚠️ [accounting/searchtimerecordemployee] workplace ${record.workplaceId} ไม่มีการตั้งค่า workTimeDay - ถือว่าทุกวันเป็น work`);
                newDayType = 'work';
                newCashWorkMul = '1';
              }
              ...
              console.error(`❌ [accounting/searchtimerecordemployee] ไม่สามารถดึงข้อมูล workplace ${record.workplaceId}:`, workplaceError.message);
