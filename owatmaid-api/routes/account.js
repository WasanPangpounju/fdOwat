...
if (isSpecialWorkplace7Days) {
  ...
  console.log(`\n🏢 === หน่วยงานพิเศษ 7 วัน (${responseConclude.data.recordConclude[c].employeeId}) ===`);
  console.log(`📊 จำนวนวันทำงานจริง (จาก dayType): ${totalWorkDays} วัน`);
  
  // แสดงรายละเอียดการนับเพื่อตรวจสอบ
  console.log(`\n📋 รายละเอียดการนับวัน:`);
  console.log(`┌─────────┬──────────┬───────────┬─────────┐`);
  console.log(`│ วันที่   │ dayType  │ allTimes  │ นับ/ไม่นับ │`);
  console.log(`├─────────┼──────────┼───────────┼─────────┤`);
  ...
  responseConclude.data.recordConclude[c].concludeRecord.forEach((record, index) => {
    ...
    const counted = hasDayType ? '✅ นับ' : '❌ ไม่นับ';
    const dayType = record.dayType || 'ไม่มี';
    const allTimes = record.allTimes || '0';
    
    console.log(`│ ${record.day.padEnd(7)} │ ${dayType.padEnd(8)} │ ${allTimes.padEnd(9)} │ ${counted.padEnd(9)} │`);
  });
  
  console.log(`└─────────┴──────────┴───────────┴─────────┘`);
  console.log(`\n📊 สรุป: จำนวนวันที่นับได้ = ${totalWorkDays} วัน`);
}

if (isSpecialWorkplace7Days) {
  addSalaryDayArray = addSalaryDayArray.map(item => ({
    ...item,
    message: totalWorkDays.toString()  // อัปเดต message เป็นจำนวนวันที่มี dayType
  }));
  console.log(`🔧 อัปเดต addSalaryDayArray message เป็น: ${totalWorkDays} (นับจาก dayType)`);
  
  // 🔴 เพิ่ม log เพิ่มเติมตรงนี้
  console.log(`\n💰 === อัปเดต message ในเงินเพิ่มพิเศษรายวัน ===`);
  console.log(`📝 จำนวนรายการเงินเพิ่มรายวัน: ${addSalaryDayArray.length} รายการ`);
  
  // แสดงค่า message หลังอัปเดต
  console.log(`\n📋 ค่า message หลังอัปเดต (ทั้งหมดเป็น ${totalWorkDays}):`);
  addSalaryDayArray.forEach((item, index) => {
    console.log(`   ${index + 1}. ${item.name} (ID: ${item.id}): message = "${item.message}"`);
    console.log(`      💵 จำนวนเงิน: ${item.SpSalary} บาท`);
  });
}

  // เพิ่มการตรวจสอบเพิ่มเติม:

  // เพิ่ม log สรุปจำนวนวันที่มี dayType
  console.log(`\n📊 === สรุป addSalaryList สำหรับหน่วยงานพิเศษ 7 วัน ===`);
  let countDaysWithDayType = 0;

  responseConclude.data.recordConclude[c].concludeRecord.forEach((record, index) => {
    if (record.dayType && record.dayType.trim() !== '') {
      countDaysWithDayType++;
    }
  });

  console.log(`📅 จำนวนวันที่มี dayType: ${countDaysWithDayType} วัน`);
  console.log(`💵 จำนวน addSalaryDayArray: ${addSalaryDayArray.length} รายการ`);
  console.log(`✅ ต้องตรงกัน: ${countDaysWithDayType === totalWorkDays ? 'ถูกต้อง' : 'ไม่ตรงกัน!'}`);
