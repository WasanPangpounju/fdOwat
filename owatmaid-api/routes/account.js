...
const calculateCashValues = async (employeeId, employee_record, month, year) => {
  ...
  const addSalary = employeeProfile?.[0]?.addSalary || [];
  ...
  if (salaryTmp > 1660) {
    salaryMonth = salaryTmp;
    salary = parseFloat((salaryTmp / 30 / 8).toFixed(3));
  } else {
    salary = parseFloat((salaryTmp / 8).toFixed(3));
  }

  // Use a synchronous loop for clarity and accuracy
  for (const record of employee_record) {
    ...
    if (record?.dayType) {
      switch (record.dayType) {
        ...
        case 'work':
          dayWorkCount += 1;
          sumTimeWork += parseFloat(record.totalTime || '0');
          sumTimeOt += parseFloat(record.beforeTotalOtTime || '0') + parseFloat(record.totalOtTime || '0');
          ...
          if (Array.isArray(record.addSalaryDaily)) {
            record.addSalaryDaily.forEach((salaryItem) => {
              const amount = parseFloat(salaryItem.SpSalary);
              if (isNaN(amount)) return; // Skip if invalid number
  ...
  // Add monthly salaries clearly
  const monthlySalaries = addSalary.filter(salary => salary.roundOfSalary === 'monthly');

  addSalaryList = addSalaryList.concat(
    monthlySalaries.map(salary => ({
      ...salary,
      SpSalary: parseFloat(parseFloat(salary.SpSalary).toFixed(2))
    }))
  );
...
const calculateCashValues_back = async (employeeId, employee_record, month, year) => {
  const employeeProfile = await getEmployeeProfile(employeeId);
const salaryTmp = parseFloat(employeeProfile[0].salary || '0') || 0;
