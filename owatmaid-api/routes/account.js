...
router.post('/checkAndUpdateDayType', async (req, res) => {
  try {
    ...
    console.log(`📊 ข้อมูลหน่วยงาน: workOfWeek = ${workplaceData.workOfWeek}, workplaceName = ${workplaceData.workplaceName}`);
    
    if (workplaceData.workOfWeek !== "7") {
      console.log(`❌ หน่วยงาน ${workplaceId} มี workOfWeek: ${workplaceData.workOfWeek} วัน (ไม่ใช่ 7 วัน)`);
    ...
    console.log(`✅ หน่วยงาน ${workplaceId} มี workOfWeek: 7 วัน - ดำเนินการต่อ`);

    // ขั้นตอนที่ 2: ดึงข้อมูล timerecord
    console.log(`🔍 กำลังดึงข้อมูล timerecord สำหรับ ${year}-${month}...`);
    ...
    console.log(`📋 พบข้อมูลพนักงาน: ${employeeRecords ? employeeRecords.length : 0} คน`);
    ...
    console.log(`🔍 กำลังตรวจสอบวันหยุดสำหรับ ${year}-${month.padStart(2, '0')}...`);
    ...
    console.log('📅 ข้อมูลวันหยุดที่ได้รับ:', JSON.stringify(weekendData, null, 2));

    // สร้าง Set ของวันที่ที่ต้องเปลี่ยนเป็น stop
    const daysToStop = new Set();
    
    if (weekendData.dayOffOnly && Array.isArray(weekendData.dayOffOnly)) {
      console.log(`🔴 พบ dayOffOnly: ${weekendData.dayOffOnly.length} วัน`);
      weekendData.dayOffOnly.forEach(date => {
        const day = new Date(date).getDate().toString();
        console.log(`  - เพิ่มวันที่ ${day} (dayOffOnly)`);
        daysToStop.add(day);
      });
    } else {
      console.log('⚪ ไม่พบ dayOffOnly หรือไม่ใช่ array');
    }
    
    if (weekendData.weekendAndDayOff && Array.isArray(weekendData.weekendAndDayOff)) {
      console.log(`🔴 พบ weekendAndDayOff: ${weekendData.weekendAndDayOff.length} วัน`);
      weekendData.weekendAndDayOff.forEach(date => {
        const day = new Date(date).getDate().toString();
        console.log(`  - เพิ่มวันที่ ${day} (weekendAndDayOff)`);
        daysToStop.add(day);
      });
    } else {
      console.log('⚪ ไม่พบ weekendAndDayOff หรือไม่ใช่ array');
    }

    console.log(`🎯 รวมวันที่ต้องเปลี่ยนเป็น stop: ${daysToStop.size} วัน [${Array.from(daysToStop).join(', ')}]`);

    // ขั้นตอนที่ 4: อัพเดท dayType
    console.log('🔄 กำลังอัพเดท dayType...');
    let updatedCount = 0;
    
    for (const employeeRecord of employeeRecords) {
      let hasUpdates = false;
      console.log(`👤 ตรวจสอบพนักงาน: ${employeeRecord.employeeName} (${employeeRecord.employee_record.length} บันทึก)`);
      
      for (const record of employeeRecord.employee_record) {
        if (record.dayType === "work" && daysToStop.has(record.date)) {
          console.log(`  ✏️  อัพเดท วันที่ ${record.date} จาก work เป็น stop`);
          record.dayType = "stop";
          hasUpdates = true;
          updatedCount++;
        }
      ...
      if (hasUpdates) {
        ...
        console.log(`  ⚪ ไม่มีการเปลี่ยนแปลงสำหรับพนักงาน ${employeeRecord.employeeName}`);
      }
    }

    console.log(`✅ เสร็จสิ้นการอัพเดท: ${updatedCount} บันทึก`);
    ...
    console.error('❌ Error in checkAndUpdateDayType:', error.message);
