    ...
    for (const doc of records) {
      ...
      try {
        ...
        try {
          ...
          console.log(`🔍 Found prefix for ${doc.employeeId}: ${employeePrefix}, workplace: ${employeeWorkplaceId}`);
        } catch (prefixError) {
          console.warn(`⚠️ Could not fetch prefix for employee ${doc.employeeId}:`, prefixError.message);
        }
        
        // ถ้าพนักงานอยู่ในหน่วยงานที่ทำงาน 7 วัน/สัปดาห์ ให้เปลี่ยน dayType เป็น "work" ทั้งหมด
