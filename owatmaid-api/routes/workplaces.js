...
// เปลี่ยน dayType เป็น work ทั้งหมดสำหรับหน่วยงานที่ทำงาน 7 วัน
router.post('/updateAllDayTypeToWork', async (req, res) => {
  try {
    ...
    if (!workplaceId || !month || !year) {
      return res.status(400).json({
        success: false,
        message: 'กรุณาระบุ workplaceId, month และ year'
      });
    }
    
    console.log(`🔍 กำลังตรวจสอบหน่วยงาน ${workplaceId} สำหรับเดือน ${month}/${year}...`);
    
    // 1. ตรวจสอบว่าหน่วยงานมี workOfWeek = "7" หรือไม่
    const workplace = await Workplace.findOne({ workplaceId });
    
    if (!workplace) {
      return res.status(404).json({
        success: false,
        message: `ไม่พบหน่วยงาน ${workplaceId}`
      });
    }
    
    console.log(`📊 หน่วยงาน ${workplaceId} (${workplace.workplaceName}) มี workOfWeek: "${workplace.workOfWeek || 'ไม่ระบุ'}" วัน`);
    
    if (workplace.workOfWeek !== "7") {
      console.log(`❌ หน่วยงาน ${workplaceId} ไม่ได้ทำงาน 7 วัน/สัปดาห์ (workOfWeek: ${workplace.workOfWeek})`);
      return res.status(200).json({
        success: false,
        message: `หน่วยงาน ${workplaceId} ไม่ได้ทำงาน 7 วัน/สัปดาห์ (workOfWeek: ${workplace.workOfWeek})`,
        workplaceName: workplace.workplaceName
      });
    }
    
    console.log(`✅ หน่วยงาน ${workplaceId} ทำงาน 7 วัน/สัปดาห์ - กำลังดำเนินการปรับ dayType...`);
    
    // 2. เรียก API searchtimerecordemployee เพื่อดึงข้อมูลพนักงาน
    ...
    const searchUrl = `${baseUrl}/accounting/searchtimerecordemployee`;
    console.log(`🔍 กำลังเรียก API: ${searchUrl}`);
    
    const timerecordResponse = await axios.post(searchUrl, timerecordData);
    const employeeRecords = timerecordResponse.data.result;
    
    if (!employeeRecords || employeeRecords.length === 0) {
      console.log(`❌ ไม่พบข้อมูลพนักงานในเดือน ${month}/${year}`);
      return res.status(200).json({
        success: false,
        message: `ไม่พบข้อมูลพนักงานในเดือน ${month}/${year}`
      });
    }
    
    console.log(`📋 พบข้อมูลพนักงานทั้งหมด ${employeeRecords.length} คน`);
    
    // 3. เปลี่ยน dayType เป็น "work" ทั้งหมด
    let totalUpdated = 0;
    let totalRecords = 0;
    
    for (const employeeRecord of employeeRecords) {
      const employeeName = employeeRecord.employeeName;
      const employeeId = employeeRecord.employeeId;
      
      console.log(`👤 กำลังปรับข้อมูลพนักงาน: ${employeeName} (${employeeId})`);
    ...
    console.log(`✅ สรุป: ปรับ dayType ทั้งหมด ${totalUpdated} รายการ จาก ${totalRecords} รายการ`);
    
    // 4. ส่งข้อมูลกลับ
    res.status(200).json({
      success: true,
      message: `ปรับ dayType เป็น "work" ทั้งหมด ${totalUpdated} รายการ สำเร็จ`,
      data: {
        workplaceId: workplaceId,
        workplaceName: workplace.workplaceName,
      ...
      // ส่งข้อมูลที่ปรับแล้วกลับไป
      result: employeeRecords
    });
    
  } catch (error) {
    console.error('❌ Error in updateAllDayTypeToWork:', error.message);
